...
def multiple_walks(L,p,d,pot,f,n,m): #boxsize, p value, density of pacmans, length of the walk, food parameter, number of walks, number of matrices
    ...
    for k in range(m): 
        ...
        for i in range(n): #average over the n walks
            ...
            msd = msd + rw(mat, perclabel, clustersize, N, startconf, pot, f)[0]
            #print(msd)
            fpercentage = fpercentage + rw(mat, perclabel, clustersize, N, startconf, pot, f)[1]
        for j in range(N):
            avgmsd += msd[j]
        avgmsd = avgmsd / (N*n)
        #print("avgmsd: ", avgmsd)
...
msd, grid, fpercentage, diff = multiple_walks(100, pc, 0, 3, 0, 100, 1)
print(msd)
diff_avg = diff.copy()   
...
plt.subplot(3,1,1)
plt.loglog(grid,msd,label='$simu$',lw = 0.5)
plt.loglog(grid,np.array(grid)**(0.7),label='$reference$',lw = 0.5)
